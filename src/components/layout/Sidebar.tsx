...
export default function Sidebar() {
  ...
  const getDivCount = (div: Division) => proposals.filter((p) => p.division === div).length;
  const getCatCount = (cat: Category) => proposals.filter((p) => p.category === cat).length;

  const roleLabel =
    user?.role === "admin" ? "Admin" : user?.role === "approver" ? "Approver" : "Proposer";
  const roleColor =
    user?.role === "admin" ? "text-red" : user?.role === "approver" ? "text-corporate" : "text-sme";
  ...
  return (
    <aside className="w-[220px] min-h-screen bg-surface border-r border-border flex flex-col flex-shrink-0">
      ...
      {pathname === "/dashboard" && (
        <div className="flex-1 sidebar-scroll px-3 py-3 space-y-4 overflow-y-auto">
          {/* Divisions */}
          <div>
            <button
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <span className={cn("w-2 h-2 rounded-full flex-shrink-0", DIV_COLORS[div])} />
